...
- **Foreign-key column names follow the referenced table.** A column referencing `<table>(id)` is named `<table_singular>_id` — e.g. a column referencing `family_members(id)` is `family_member_id`, a column referencing `auth.users(id)` is `user_id`. Exception: when a single table has multiple FKs to the same target, use a role-based name instead (e.g. `booked_by`, `requested_by_member_id`, `decided_by_member_id`) so the relationship's purpose is obvious at the call site
